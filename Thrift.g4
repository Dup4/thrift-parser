...
include_: 'include' LITERAL_VALUE;

namespace_:
	'namespace' '*' (IDENTIFIER | LITERAL_VALUE)
	| 'namespace' IDENTIFIER (IDENTIFIER | LITERAL_VALUE) type_annotations?
	| 'cpp_namespace' IDENTIFIER
	| 'php_namespace' IDENTIFIER;

cpp_include: 'cpp_include' LITERAL_VALUE;
...
struct_: 'struct' IDENTIFIER '{' field* '}' type_annotations?;

union_: 'union' IDENTIFIER '{' field* '}' type_annotations?;
...
annotation_value: integer | LITERAL_VALUE;

field_type: base_type | IDENTIFIER | container_type;

base_type: real_base_type type_annotations?;

container_type: (map_type | set_type | list_type) type_annotations?;

map_type: 'map' cpp_type? '<' field_type COMMA field_type '>';

set_type: 'set' cpp_type? '<' field_type '>';

list_type: 'list' '<' field_type '>' cpp_type?;

cpp_type: 'cpp_type' LITERAL_VALUE;
